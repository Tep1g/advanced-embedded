...
class UnidirectionalMotor():
    ...
    _SEC_TO_NS_FLOAT = const(1_000_000_000)
    _PWM_FREQ = const(20_000)
    _PWM_DUTY_NS = const(_SEC_TO_NS_FLOAT / _PWM_FREQ)
    
    def __init__(self, pwm_gpio: int):
        self._pwm = PWM(Pin(pwm_gpio, Pin.OUT))
        self._pwm.duty_ns(0)
        self._pwm.freq(self._PWM_FREQ)
        self._duty_ns = round(self._SEC_TO_NS_FLOAT / self._PWM_FREQ)
    ...
    def set_speed_pct(self, speed_pct: int):
        ...
        self._pwm.duty_ns(self._PWM_DUTY_NS * speed_pct / 100)
